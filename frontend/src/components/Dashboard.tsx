...
import { Upload, Settings, Activity, ShieldCheck, AlertTriangle, FileText, ChevronRight, CheckCircle2, BarChart2, PieChart, Download } from "lucide-react";
import { 
...
- **Mitigation Strategy:** ${config.mitigation}
- **Target Column:** ${config.target_col}
- **Sensitive Attribute:** ${config.sensitive_col}

## Key Metrics
- **Fairness Score:** ${(results.metrics.fairness_score * 100).toFixed(1)}%
- **Base Accuracy:** ${(results.metrics.accuracy_base * 100).toFixed(1)}%
- **Mitigated Accuracy:** ${(results.metrics.accuracy_mitigated * 100).toFixed(1)}%
- **Accuracy Drop:** ${((results.metrics.accuracy_mitigated - results.metrics.accuracy_base) * 100).toFixed(1)}%
- **Demographic Parity Diff (Before):** ${results.metrics.dpd_before.toFixed(3)}
- **Demographic Parity Diff (After):** ${results.metrics.dpd_after.toFixed(3)}

## Group Analysis
${results.group_table.map((g: any) => `- **${g.group}**: Status: ${g.bias_flag} | Rate Before: ${(g.rate_before * 100).toFixed(1)}% | Rate After: ${(g.rate_after * 100).toFixed(1)}%`).join('\n')}

---
*Generated by FairAI Audit System*
    `.trim();
    ...
    a.href = url;
    a.download = \`fairness_report_\${new Date().getTime()}.md\`;
              ...
              <div className="flex items-center justify-between mb-8">
                ...
                <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                  ...
                  <button 
                    ...
                    className="flex items-center justify-center gap-2 px-4 py-3 sm:py-2 bg-indigo-500 hover:bg-indigo-400 text-white text-sm font-medium rounded-xl transition-all shadow-lg shadow-indigo-500/20 h-full w-full sm:w-auto"
                  >
